...
const playOnce = (element, callback, eventType = 'animationend', action = playAnimation) => {
    let afterAnimate = function(event) {
...
const afterFirstIteration = (element, callback) => {
...
const showText = (text, long = false) => {
    overlayContent.innerHTML = text;

    overlay.classList.toggle('long-text', long);

    showOverlay();
}

const showIntro = (index, callback) => {
    let stage = document.querySelector(`.stage-${index}`),
    ...
    playOnce(intro, () => {
        ...
        let loop = stage.querySelector(':scope > .loop');
...
const showStage = (index) => {
    if (index > 0) {
        clearElement(document.querySelector(`.stage-${index-1}`));
    }
...
const showResultText = (result, text) => {
    let overlay;
...
const showResult = (stageIndex, action) => {
    let layer = document.querySelector(`.stage-${stageIndex} .${action}`),
        animatable,
        loop;
    ...
    playOnce(animatable, () => {
        ...
        let result = scenario[stageIndex][action].result,
            text = scenario[stageIndex][action].text[currentLanguage];

        if (loop) {
            afterFirstIteration(loop, () => {
...
const presentationButtonClick = (event) => {
    let target = event.currentTarget;

    if (target.classList.contains('start-game')) {
        startGame();
    } else if(target.hasAttribute(`data-text-${currentLanguage}`)) {
        showPresentationOverlay(target.getAttribute(`data-text-${currentLanguage}`));
    }
}
...
const showPresentationOverlay = (text) => {
...
okButton.addEventListener('click', (event) => {
    if (gameStarted) {
        ...
        let startGate = document.querySelector('.start');
...
Array.prototype.forEach.call(presentationButtons, (item, index, array) => {
...
Array.prototype.forEach.call(langButtons, (item) => {
